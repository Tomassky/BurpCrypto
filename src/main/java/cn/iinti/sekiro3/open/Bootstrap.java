...
public class Bootstrap {
    ...
    private static final NioEventLoopGroup serverBossGroup = new NioEventLoopGroup(Runtime.getRuntime().availableProcessors() * 2, new DefaultThreadFactory("sekiro-boss"));

    private static final NioEventLoopGroup serverWorkerGroup = new NioEventLoopGroup(Runtime.getRuntime().availableProcessors() * 6, new DefaultThreadFactory("sekiro-worker"));
    ...
    public static void shutdown(){
        if (serverBossGroup != null && serverWorkerGroup != null) {
            try {
                serverBossGroup.shutdownGracefully();
                serverWorkerGroup.shutdownGracefully();
                System.out.println("sekiro netty server has been stopped");
                log.info("sekiro netty server has been stopped");
            } catch (Exception e) {
                System.out.println(e);
                log.error("Error shutting down sekiro netty server", e);
            }
        }
    }
}
